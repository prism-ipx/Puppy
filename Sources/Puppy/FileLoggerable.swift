...
extension FileLoggerable {
  ...
  func validateFolderURL(_ url: URL) throws {
    if !url.hasDirectoryPath {
      throw FileError.isNotDirectory(url: url)
    }
  }

  func validateFolderPermission(_ url: URL, folderPermission: String) throws {
    let min = UInt16("000", radix: 8)!
    let max = UInt16("777", radix: 8)!
    if let uintPermission = UInt16(folderPermission, radix: 8), uintPermission >= min,
      uintPermission <= max
    {
    } else {
      throw FileError.invalidFolderPermission(at: url, folderPermission: folderPermission)
    }
  }
  ...
  func validateFilePermission(_ url: URL, filePermission: String) throws {
    let min = UInt16("000", radix: 8)!
    let max = UInt16("777", radix: 8)!
    if let uintPermission = UInt16(filePermission, radix: 8), uintPermission >= min,
      uintPermission <= max
    {
    } else {
  ...
  func append(
    _ level: LogLevel, string: String, flushMode: FlushMode = .always,
      ...
      if let data = (string + "\r\n").data(using: .utf8) {
        ...
        case .assert, .print:
          do {
            ...
            let message =
              "error in appending data in a file, error: \(error.localizedDescription), file: \(fileURL)"
            if writeMode == .assert {
